...
##trying to do some sensttivity analysis for differnt meterologic al forcing :

##precipitation change and temperature change in combined:
banum=c(29,113,25,135,131,43)

##change the corresponidng watershed directory and basin number and run below code

regime_change=function(delP,delT){
  
  #regime_change=function(delP,delT) 
  
  dir=("F:\\USU_Research_work_update_March_30_2014\\RAINFALL_COMPARISON\\C22_watershed\\PRISM_RUN")
  setwd(dir)
  ...
  dateT=temp[,(ncol(temp)-1)]
  hourT=temp[,(ncol(temp))]
  max_temp=temp[,seq(1,(ncol(temp)-2),3)]+delT
  min_temp=temp[,seq(2,(ncol(temp)-2),3)]+delT
  dew_temp=temp[,seq(3,(ncol(temp)-2),3)]+delT
  ncT=((ncol(temp))-2)/3
  ...
  temper=data.frame(temper,dateT,hourT)
  ...
  cat("\n", file="tmaxtmintdew.dat", append=TRUE)
  sites=seq(1,(ncT),1)
  cat(sprintf("%s %d ", "ver2",ncT),file='tmaxtmintdew.dat',append=TRUE) 
  cat(sprintf( "%d", sites),(sprintf( "%s", "Date Hour","\n")),file='tmaxtmintdew.dat',append=TRUE) 
  cat("\n", file="tmaxtmintdew.dat", append=TRUE)
  ...
  time_his=seq(as.Date("2000/1/1"), as.Date("2010/12/31"), "day")
  time_his1=seq(as.Date("2001/10/1"), as.Date("2010/9/30"), "day")
...
#c=seq(1.4,0.6,-0.2)
#df <- expand.grid(x=a, y=c,z=b)

##save results in a text file for each watershed and each forcing
write.table(rg,"C22_PT_PR_sensi.txt",row.names=FALSE,col.names=FALSE) ## A1_PT_DY_sensi.txt== A1 watershed precipitation and temperature sensitivity combined for the Daymet data sets.


             ## plotting options

dir="F:\\USU_Research_work_update_March_30_2014\\RAINFALL_COMPARISON\\Results\\Sensitivity_Results"
setwd(dir)

rain_temp_files=list.files(path=dir,pattern="*_sensi.txt")
